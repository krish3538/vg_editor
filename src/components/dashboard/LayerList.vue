<template>
  <div class="w-64 flex-shrink-0 border-r border-zinc-700 layers_box"> <!--  :style="{ width: `${DRAWER_WIDTH}px` }"-->
    <div class="flex items-center border-zinc-700 border-y pl-4 py-3 text-zinc-500 select-none tracking-widest">
      LAYERS
    </div>
    <template v-if="layers.length">
      <div
        class="pl-4 cursor-pointer layer-item flex items-center border-b-2 border-zinc-800"
        v-for="(layer, i) of [...layers].reverse()"
        :class="{
