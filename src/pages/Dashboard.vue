<template>
  <!-- <canvas
    ...
    ref="recordCanvas"
  /> -->
  <div class="w-full flex text-white" style="height: calc(100% - var(--timeline-height-full))">
    <sidebar class="w-3/12" />
    <div class="main w-7/12 h-full flex flex-col justify-center items-center relative">
      <!-- <memory-display />
      <div class="top-right flex flex-col">
        ...
        <div class="mt-3 flex justify-center bg-zinc-800 py-1 rounded-md select-none">
          🔎 {{ `${state.zoomLevel.toFixed(0)}%` }}
        </div>
      </div> -->
      <div class="bottom bg-black bg-opacity-50 px-4 py-2 rounded-lg z-10 text-2xl flex justify-between items-center">
...
import {
  ...
  TIME_OPTIONS, INITIAL_CANVAS_FORMAT
...
const state: State = reactive({
  ...
  outputFormat: OUTPUT_FORMAT_OPTIONS[0].value,
  newLayer: null,
  dragging: false,
  trimming: false,
  // NOTE: Miliseconds
  ...
  canvasFormat: INITIAL_CANVAS_FORMAT.format,
});
...
//     createToast("MediaRecorder API is not supported in this browser.", colors.red.darken1);
//     return;
//   }
 
//   // Initialize chunks array to store video data
//   const chunks: Blob[] = [];
//   const stream = recordCanvas.value!.captureStream(60); // Captures the canvas stream at 60fps
 
//   // Configure MediaRecorder
//   recorder = new MediaRecorder(stream, {
//     mimeType: "video/webm;codecs=vp9", // Use webm format with vp9 codec
//     bitsPerSecond: 3200000, // Adjust bitrate as needed
//   });
 
//   // Capture data chunks as they are available
...
//       a.href = url;
//       a.download = "output.webm"; // Set the file name
//       a.click();
...
//   //   console.log("FFmpeg Installed:", installed);
//   //   if (!installed) {
//   //     createToast("Cannot render, ffmpeg is not installed!", colors.red.darken1);
//   //     return;
//   //   }
//   // } catch (err) {
//   //   console.log("Error checking FFmpeg installation:", err);
//   //   createToast("Error checking FFmpeg installation.", colors.red.darken1);
//   //   return;
//   // }
//   const chunks: Blob[] = [];
//   const stream = recordCanvas.value!.captureStream(30);
//   recorder = new MediaRecorder(stream, {
  ...
  if (!!layers.value.length) {
    if (paused.value) {
      state.playInterval = setInterval(() => {
        // Use the duration from dashboard store
        const videoDuration = dashboardStore.videoDuration;
        if (state.currentTime + 1 > videoDuration) {
          seekToStart();
          togglePlay();
          return;
        }
        ...
        videoObjects.value.forEach((v) => {
          const videoObject = v.object as fabric.Image & { getElement(): HTMLVideoElement };
          const element = videoObject.getElement() as HTMLVideoElement;
...
const newSvg = (path: string) => {
  ...
  const filename = path.split('/').pop()?.replace('.svg', '') || 'svg';
...
const newImage = async (source: File | string) => {
  ...
  if (typeof source !== "string") {
    ...
    // Extract filename from path for static images
...
const newVideo = (file: HTMLVideoElement, source: string, duration: number) => {
  const id = randInt(1, 9999).toString();
  const filename = source.split('/').pop()?.replace('.mp4', '') || 'video';

  // Video dimensions and aspect ratio
  const videoWidth = file.videoWidth;
  const videoHeight = file.videoHeight;
  const videoRatio = videoWidth / videoHeight;

  // Canvas dimensions and aspect ratio
  const canvasWidth = artboardWidth.value;
  const canvasHeight = artboardHeight.value;
  const canvasRatio = canvasWidth / canvasHeight;

  // Determine the scale to fit the video within the canvas
  let scale;
  if (videoRatio > canvasRatio) {
    // Video is wider than the canvas
    scale = canvasWidth / videoWidth;
  } else {
    // Video is taller or has equal aspect ratio as the canvas
    scale = canvasHeight / videoHeight;
  }

  // Create the fabric image object
  const newVideo = new fabric.Image(file, {
    left: artBoardLeft.value + canvasWidth / 2,
    top: artBoardTop.value + canvasHeight / 2,
    originX: 'center',
    originY: 'center',
...
const dragObjectProps = ({ offsetX }: MouseEvent, layer: Layer) => {
  ...
  let action = "dragging";
  const layerWidth = layer.duration / 1 - layer.endTrim - layer.startTrim;
  if (offsetX <= 7) {
    action = "trimLeft";
  ...
  const dragging = ({ pageX }: MouseEvent) => {
    ...
    console.log(`End Time: ${endTime.toFixed(2)}s`);
...
// Hooks
onMounted(() => {
  dashboardStore.setArtboardDimensions(640, 360);
  ...
  fabricCanvas.renderAll();
  createToast("🎉 Welcome to Videograph Editor!", colors.purple.darken1);
});
